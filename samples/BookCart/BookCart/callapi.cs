...
// Note: In a real scenario, we'd extract the JWT token from the response body.
// For this demo, we assume the API returns a token or we simulate the header.
string token = "mock_token_from_login_response"; 
http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", token);
Console.WriteLine($"[POST] Admin Login: {loginRes.StatusCode}");
